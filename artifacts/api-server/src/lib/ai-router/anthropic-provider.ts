...
export const ANTHROPIC_RELOCATION_MODEL = "claude-sonnet-4-6";
...
export async function generateRelocationSummaryWithClaude(
  ...
  try {
    ...
    return { output: validated, model: ANTHROPIC_RELOCATION_MODEL };
  } catch (err) {
    logger.warn({ err }, "Claude relocation summary failed — will use deterministic fallback");
    return null;
  }
}
